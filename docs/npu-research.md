...
- 当初の推測（qkv の 5 次元 reshape → permute → Gather と、k の転置・バイアス定数の展開が
  並べ替えコストになっている）は、下の 1〜2 ブロック実験で否定された。

attention の書き換え実験（同日。layers.2 の blocks.0→blocks.1 を切り出した 2 ブロックモデル、
...
| V4a: 相対位置バイアス加算を除去（切り分け専用） | 0.328 秒 | 36 ms の層は消えない |

- 切り出したモデルでは、qkv の融合層は 3〜4 ms、`Transpose+MatMul+Add` は 3.5 ms しかかからない。
...
- 同一グラフの再コンパイルによる変動（±8%、V3 では 0.06 秒）と同程度の効果で、全体モデルの
  再コンパイル（約 1 時間）とキャッシュ・配布物の差し替えに見合わないため、全体への適用は見送った。
  実験一式は `tmp/npu-perf/swinir-attn/`（リポジトリ外）。
