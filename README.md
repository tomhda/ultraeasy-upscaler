...
**Q1: PCを専有していい？（=速さ最優先）→ GPU**

| コンテンツ | 選択 | 実効 |
|---|---|---|
| アニメ/CG | GPU + Anime Video v3 | **0.7秒/枚**・画質もアニメ最強 |
| 実写 | GPU + General（ノイズ除去弱） | 同速クラス・自然な仕上がり |

**Q2: PCを使いながら／夜間放置／発熱を抑えたい → NPU**

| コンテンツ | 選択 | 実効 |
|---|---|---|
| アニメ/CG | **NPU + Anime Video v3（bf16）** | 3.1秒/枚・GPU版と同じ絵がGPUゼロで出る |
| 実写 | NPU + Real-ESRGAN（bf16） | 3.7秒/枚・fp16級画質 |
| アニメで質感重視の代替 | NPU + Real-ESRGAN Anime（bf16） | 4.5秒/枚 |

**静止画1枚勝負** → GPU + Real-ESRGAN（時間を気にしない前提）

各バックエンドの★推奨（アニメ/実写）はアプリのモデルピッカーにも表示される。
